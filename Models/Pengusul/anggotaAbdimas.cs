...
{
    public class anggotaAbdimas : _abstractModels
    {
        ...
        {
            bool isSuccess = false;
            string strSQL = string.Format("SELECT * FROM hibah.list_peran_personil_pengabdian_rb('{0}');", idUsulanKegiatan);
            isSuccess = this._db.FetchDataTable(strSQL, ref dataTable);
            if (!isSuccess)
                this._errorMessage = this._db.ErrorMessage;
        ...
        public bool deleteData(Guid idPersonil)
        {
            bool isSuccess = false;
        ...
        public bool listAnggotaPersonil(ref DataTable dataTable, Guid idUsulanKegiatan, Guid idPersonil)
        {
            bool isSuccess = false;
