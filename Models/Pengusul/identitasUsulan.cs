...
{
    public class identitasUsulan : _abstractModels
    {
        #region Fields
        ...
        public bool getIdentitasUsulanLanjutan(ref DataTable dataTable, string idPersonal, string thnUsulan,
            string thnPelaksanaan)
        {
            bool isSuccess = false;

            string strSQL = string.Format("SELECT * FROM hibah.list_usulan_lanjutan_personal ('{0}','{1}','{2}',0,0);",
                idPersonal, thnUsulan, thnPelaksanaan);
        ...
        public bool listRiwayatUsulan(ref DataTable dataTable, string idPersonal, int offset)
        {
            ...
            string strSQL = string.Format("SELECT * FROM hibah.list_riwayat_usulan_personal('{0}',{1},{2});",
        ...
        {
            bool isSuccess = false;
            string strSQL = @"select * from hibah.insup_identitas_usulan_penelitian (
                            @p_id_usulan,
            ...
            isSuccess = this._db.ExecuteNonQuery(strSQL
                    ...
                    , new Npgsql.NpgsqlParameter("@p_id_rumpun_ilmu", p_id_rumpun_ilmu)
                    , new Npgsql.NpgsqlParameter("@p_id_topik_unggulan_perguruan_tinggi",
                            (object)p_id_topik_unggulan_perguruan_tinggi ?? DBNull.Value)
                    , new Npgsql.NpgsqlParameter("@p_id_topik_rirn",
                            (object)p_id_topik_rirn ?? DBNull.Value)
        ...
        public bool insertDataLanjutanPengabdianRb(ref string idUsulanKegiatan, Guid idPersonal,
            Guid idUsulan, int urutanTahun, string thnUsulan, string thnPelaksanaan, int idSkema)
        {
            bool isSuccess = false;
            string strSQL = @"SELECT * FROM hibah.insert_usulan_lanjutan_abdimas (
                              :p_id_personal::uuid,
                              :p_id_usulan::uuid,
                              :p_urutan_thn_kegiatan::smallint,
                              :p_thn_usulan_kegiatan::char(4),
        ...
        public bool getSkemaKegiatanByTKT(ref DataTable dataTable, string kdProgramHibah, int levelTKT,
            Guid idPersonal, string status)
        {
            bool isSuccess = false;
            string query;
            if (status == "1")
                query = $@"SELECT * FROM referensi.list_skema_kegiatan_by_level_tkt('{kdProgramHibah}',{levelTKT},'{idPersonal}') where id_skema not in (6,30,52,76) ;";
            else if (status == "2")
            { query = $@"SELECT * FROM referensi.list_skema_kegiatan_by_level_tkt('{kdProgramHibah}',{levelTKT},'{idPersonal}') where id_skema in (6,30,52,76) ;"; }
            else
            {
                query = $@"SELECT * FROM referensi.list_skema_kegiatan_by_level_tkt('{kdProgramHibah}',{levelTKT},'{idPersonal}');";
            }
        ...
        public bool getSkemaKegiatanByTKT(ref DataTable dataTable, string kdProgramHibah, int levelTKT
           , string thnUsulan, string thnPelaksanaan, Guid idPersonal, string status)
        {
            bool isSuccess = false;
            string query;
            if (status == "1")
                query = $@"SELECT * FROM referensi.list_skema_kegiatan_by_personal('{kdProgramHibah}',{levelTKT},'{thnUsulan}','{thnPelaksanaan}','{idPersonal}') where id_skema not in (6,30,52,76) ;";
            else if (status == "2")
                query = $@"SELECT * FROM referensi.list_skema_kegiatan_by_personal('{kdProgramHibah}',{levelTKT},'{thnUsulan}','{thnPelaksanaan}','{idPersonal}') where id_skema in (6,30,52,76) ;";
            else if (status == "4")
                query = $@"SELECT * FROM referensi.list_skema_kegiatan_by_personal('{kdProgramHibah}',{levelTKT},'{thnUsulan}','{thnPelaksanaan}','{idPersonal}') where id_skema in (7) ;";
            else
                query = $@"SELECT * FROM referensi.list_skema_kegiatan_by_personal('{kdProgramHibah}',{levelTKT},'{thnUsulan}','{thnPelaksanaan}','{idPersonal}');";
            dataTable = new DataTable();
            isSuccess = this._db.FetchDataTable(query, ref dataTable);
            if (!isSuccess)
                this._errorMessage = this._db.ErrorMessage;
        ...
        public bool getSkemaKegiatanByTKTOngoing(ref DataTable dataTable, string kdProgramHibah, int levelTKT
           , string thnUsulan, string thnPelaksanaan, Guid idPersonal, string status)
        {
            bool isSuccess = false;
            string query;
        ...
        public bool getTktSbkbySkema(ref DataTable dataTable, int idSkema)
        {
            bool isSuccess = false;
            string query = "select st.level_tkt, st.id_kategori_sbk from referensi.get_tkt_sbk_by_skema ({0}) st;";
            query = string.Format(query, idSkema);
            dataTable = new DataTable();
            isSuccess = this._db.FetchDataTable(query, ref dataTable);
            if (!isSuccess)
                this._errorMessage = this._db.ErrorMessage;
        ...
        public bool getRumpunIlmu(ref DataTable dataTable, int level, Guid idReferensi = default(Guid))
        {
            bool isSuccess = false;

            var query = $@"SELECT id_rumpun_ilmu, rumpun_ilmu FROM referensi.rumpun_ilmu 
                           WHERE level_rumpun_ilmu = {level} AND kd_sts_aktif = '1' ";
            if (idReferensi != default(Guid))
                query = query + $"AND id_ref_rumpun_ilmu='{idReferensi}'";
        ...
        public bool getSBK(ref DataTable dataTable, int idSkema, int targetTKT)
        {
            bool isSuccess = false;

            var query = $@"SELECT * FROM referensi.list_kategori_riset_thn_implementasi_with_level_tkt_2018_rb({idSkema},{targetTKT});";
        ...
        public bool getBidangFokus(ref DataTable dataTable, int idSkema, int kdTargetTKT, int idKategoriSBK)
        {
            bool isSuccess = false;

            var query = $@"SELECT * FROM referensi.list_bidang_fokus_by_kategori_sbk_thn_implementasi_2018({idSkema},{idKategoriSBK});";
        ...
        public bool kirimUsulanPerbaikan(Guid idUsulanKegiatan)
        {
            bool isSuccess = false;

            string strSQL = @"SELECT * FROM hibah.insup_kirim_usulan_perbaikan(:p_id_usulan_kegiatan::uuid);";
        ...
        public bool checkwhitelist(ref DataTable dataTable, string idpersonal, string id_skema, string thn_usulan, string thn_pelaksanaan)
        {
            bool isSuccess = false;

            string strSQL = string.Format("SELECT * FROM hibah.cek_whitelist('{0}'::uuid,'{1}'::smallint,'{2}'::character(4),'{3}'::character(4));",
                idpersonal, id_skema, thn_usulan, thn_pelaksanaan);
        ...
        public bool checkWhitelistInstitusi(ref DataTable dataTable, string idpersonal, string thnUsulan, string thnPelaksanaan, int idSkema)
        {
            bool isSuccess = false;

            string strSQL = string.Format("SELECT * FROM hibah.cek_whitelist_institusi('{0}','{1}','{2}',{3});",
                idpersonal, thnUsulan, thnPelaksanaan, idSkema);
