...
{
    public class daftarTendikNonDosen : _abstractModels
    {
        public daftarTendikNonDosen()
        {
            setInitValues();
        }
        #region Methods
        ...
        public bool listData(Guid id_institusi, ref DataTable dataTable, string pencarian, string kd_sts_aktif)
        {
            bool isSuccess = false;
            string strSQL = string.Format(@"SELECT * FROM pdpt.get_data_tendik('{0}','{1}',{2},{3}, '{4}', '{5}');"
                            , "00000000-0000-0000-0000-000000000000", id_institusi, this.rowsPerPage, this.currentPage * this._rowsPerPage, pencarian, kd_sts_aktif);
            isSuccess = this._db.FetchDataTable(strSQL, ref dataTable);
            if (!isSuccess)
                this._errorMessage = this._db.ErrorMessage;
            return isSuccess;
        ...
        public bool getDaftarData(Guid id_institusi, string pencarian, string kd_sts_aktif)
        {
            bool isSuccess = false;

            string strSQL = @"SELECT * FROM pdpt.get_data_tendik(@id_personal, @id_institusi, @limit, @offset, @pencarian, @kd_sts_aktif);";
