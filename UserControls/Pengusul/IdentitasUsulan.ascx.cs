...
namespace simlitekkes.UserControls.Pengusul
{
    public partial class IdentitasUsulan : System.Web.UI.UserControl
    {
        ...
        public void InitIdentitasUsulan(string thn_usulan, string thn_pelaksanaan, Guid idUsulanKegiatan = default(Guid))
        {
            ...
            else
            {
                ...
                if (modelIdentitas.getIdentitasUsulan(ref dt, IdUsulan) && dt.Rows.Count > 0)
                {
                    ...
                    isiddlSBK();
                    setValueDropDownList(ref ddlSBK, dt.Rows[0]["id_kategori_sbk"].ToString());
        ...
        private void modePemilihanSkema(bool isEnabled)
        {
            ...
            rblKategoriPenelitian.Enabled = isEnabled;
            ddlPilarTransformasi.Enabled = isEnabled;
        ...
        private void isiddlTopikUnggulanPT()
        {
            ...
            if (modelIdentitas.getTopikUnggulanPenelitianPT(ref dt, Guid.Parse(ddlBidangUnggulanPT.SelectedValue)))
            {
                ...
                ddlTopikUnggulanPT.Items.Add(new ListItem { Text = "-- Pilih Topik Pilar Transformasi --", Value = "0", Selected = true });
        ...
        private void isiddlRumpunIlmu(ref DropDownList ddl, int level, Guid idReferensi = default(Guid))
        {
            ...
            if (modelIdentitas.getRumpunIlmu(ref dt, level, idReferensi))
            {
                ...
                ddl.Items.Clear();
                ddl.Items.Add(new ListItem { Text = $"-- Pilih Rumpun Ilmu Level {level} --", Value = "-1" });
                ddl.DataSource = dt;
                ddl.DataBind();
            }
        }
        ...
        {
            var dt = new DataTable();
            if (modelIdentitas.getBidangFokus(ref dt, int.Parse(ddlSkemaPenelitian.SelectedValue), int.Parse(ddlTargetTKT.SelectedValue),
        ...
        public bool Simpan()
        {
            ...
            switch (rblKategoriPenelitian.SelectedValue)
            {
                case "1":  //Desentralisasi
                    if (ddlTopikUnggulanPT.SelectedValue == "0") invalidData.Add("Topik Pilar Transformasi");
                    break;
